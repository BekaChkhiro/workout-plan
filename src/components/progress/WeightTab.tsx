...
export function WeightTab({ entries, today, targetKg }: Props) {
  ...
  const remainingKg = currentKg != null && targetKg != null ? Math.abs(currentKg - targetKg) : null;
  ...
  return (
    <div className="flex flex-col gap-3 px-5">
      ...
      {currentKg != null && (
        ...
        >
          <div className="flex items-stretch gap-4" style={{ borderBottom: "none" }}>
            {/* Left — current weight */}
            <div className="flex flex-1 flex-col gap-1">
              ...
              {deltaKg != null && daysSinceStart != null && daysSinceStart > 0 && (
                <span
                  className="rounded-pill self-start px-2.5 py-1 text-[11px] font-bold"
                  style={{ background: "#E7F8EE", color: "#2E8B57" }}
                >
                  ✨ {deltaKg <= 0 ? "−" : "+"}
                  {Math.abs(deltaKg).toFixed(1)} კგ {daysSinceStart} დღეში
                </span>
              )}
            </div>
            ...
            {targetKg != null ? (
              <div className="flex w-[40%] flex-col gap-1.5">
                ...
                {progressPct != null && (
                  <>
                    <div
                      className="rounded-pill relative h-1.5 w-full overflow-hidden"
