...
import { reorderMealsAction, toggleMealCompleteAction } from "../actions";
import { MealEditorSheet } from "./MealEditorSheet";

type MealSlot = { bg: string; color: string; emoji: string };
...
function SortableMealCard({
  ...
  const open = expanded && !isEditMode;
  ...
  return (
    <motion.li
      ...
      style={{
        ...
        opacity: isEditMode ? (isDragging ? 0.5 : 1) : completed ? 0.7 : 1,
      }}
    >
      {showConfetti ? <ConfettiBurst key={confettiKey} /> : null}
      ...
      >
        {isEditMode ? (
          <button
            ...
            {...attributes}
            {...listeners}
            className="cursor-grab touch-none text-[16px] font-extrabold tracking-[-0.06em] text-[#B7AAD0] outline-none active:cursor-grabbing"
          ...
          >
            {isPreworkout ? (
              ...
              <TimeBlock time={meal.time} label={meal.name} slot={slot} completed={completed} />
            )}
          </button>
        )}
        ...
        <button
          type="button"
          onClick={isEditMode ? undefined : onToggleExpand}
          disabled={isEditMode}
          aria-expanded={open}
          aria-controls={`meal-${meal.id}-details`}
          className="flex min-w-0 flex-1 items-center gap-3 rounded-[20px] text-left outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-brand-lilac)] disabled:cursor-default"
        >
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline gap-1.5">
              ...
              <span
                ...
                style={{
                  textDecoration: completed && !isEditMode ? "line-through" : "none",
                  color: completed && !isEditMode ? "var(--color-ink-mute)" : undefined,
                }}
              >
                {meal.name}
              </span>
          ...
          <div className="flex flex-shrink-0 flex-col items-end gap-1">
            <div className="text-ink text-[15px] leading-none font-extrabold">{meal.calories}</div>
...
export function MealsList({ meals }: { meals: MealWithDetails[] }) {
  ...
  const orderedMeals = useMemo(() => {
    if (!draggedOrder) return meals;
    const byId = new Map(meals.map((m) => [m.id, m]));
    return draggedOrder
      .map((id) => byId.get(id))
      .filter((m): m is MealWithDetails => m !== undefined);
  }, [meals, draggedOrder]);
  ...
  const biggestId = orderedMeals.reduce<MealWithDetails | null>(
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const ids = orderedMeals.map((m) => m.id);
    const oldIndex = ids.indexOf(String(active.id));
    const newIndex = ids.indexOf(String(over.id));
    const next = arrayMove(ids, oldIndex, newIndex);
    setDraggedOrder(next);
    void reorderMealsAction(next);
  }

  function handleToggle(meal: MealWithDetails) {
    const current = isCompleted(meal);
    const nextValue = !current;
    const completedNow = orderedMeals.filter((m) => isCompleted(m)).length;
    ...
    <>
      <div className="relative z-1 flex items-baseline justify-between px-[22px] pt-6 pb-3">
        ...
        <button
          ...
          style={{
            color: isEditMode ? "#7DDFA8" : "#7B4FA8",
            textDecorationLine: isEditMode ? "none" : "underline",
            textDecorationColor: "rgba(123,79,168,0.4)",
          }}
        >
          {isEditMode ? "✓ დასრულება" : "რედაქტირება"}
        </button>
      </div>

      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext
          items={orderedMeals.map((m) => m.id)}
            ...
            <ul className="relative z-1 flex flex-col gap-3 px-[18px]">
              {orderedMeals.map((meal, idx) => (
                <SortableMealCard
                  ...
                  completed={isCompleted(meal)}
                  isBiggest={meal.id === biggestId}
                  showConfetti={confettiMealId === meal.id}
                  confettiKey={confettiKey}
                  onToggleExpand={() => setExpandedId(expandedId === meal.id ? null : meal.id)}
                  onToggleDone={() => handleToggle(meal)}
